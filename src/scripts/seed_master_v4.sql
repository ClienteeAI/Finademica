...
-- SEED MASTER V4 - ROBUST DEPLOYMENT (540 QUESTIONS)
-- This script ensures the schema is correct, clears old data, and populates 540 questions.
-- This version handles the 'module' and 'level' columns in 'ai_quiz_questions' via a post-sync.
...
-- 2. CLEAN START (WIPE EXISTING TO PREVENT DUPLICATES/MISMATCHES)
DELETE FROM ai_quiz_questions;
DELETE FROM quizzes;

-- 3. POPULATE MASTER QUIZZES AND QUESTIONS
DO $$
DECLARE
...
BEGIN
    -- Ensure Quizzes exist and get IDs
    
    -- FOREX
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('forex', 'Beginner', 75, 5) RETURNING id INTO f_beg;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('forex', 'Intermediate', 75, 10) RETURNING id INTO f_int;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('forex', 'Elite', 75, 15) RETURNING id INTO f_eli;

    -- STOCKS
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('stocks', 'Beginner', 75, 5) RETURNING id INTO s_beg;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('stocks', 'Intermediate', 75, 10) RETURNING id INTO s_int;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('stocks', 'Elite', 75, 15) RETURNING id INTO s_eli;

    -- CRYPTO
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('crypto', 'Beginner', 75, 5) RETURNING id INTO c_beg;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('crypto', 'Intermediate', 75, 10) RETURNING id INTO c_int;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('crypto', 'Elite', 75, 15) RETURNING id INTO c_eli;

    -- COMMODITIES
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('commodities', 'Beginner', 75, 5) RETURNING id INTO m_beg;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('commodities', 'Intermediate', 75, 10) RETURNING id INTO m_int;
    INSERT INTO quizzes (module, level, pass_score, question_count) VALUES ('commodities', 'Elite', 75, 15) RETURNING id INTO m_eli;

    -- [PASTED QUESTIONS FROM V2]
    -- FOREX BEGINNER
    INSERT INTO ai_quiz_questions (quiz_id, question, options, correct_option, explanation) VALUES 
    (f_beg, 'What does "PIP" stand for in Forex trading?', '["Percentage in Point", "Price Interest Point", "Percentage in Price", "Private Investment Plan"]'::jsonb, 0, 'PIP stands for Percentage in Point, representing the smallest price move a given exchange rate makes.'),
    (f_beg, 'Which of the following is considered a "Major" currency pair?', '["EUR/USD", "EUR/GBP", "USD/TRY", "AUD/NZD"]'::jsonb, 0, 'EUR/USD is the most traded currency pair and belongs to the "Majors".'),
    (f_beg, 'If you are "Long" on a currency pair, what are you doing?', '["Buying the pair", "Selling the pair", "Waiting for a crash", "Hedging against inflation"]'::jsonb, 0, 'Going "Long" means buying the base currency and selling the quote currency.'),
    (f_beg, 'What is the "Spread" in Forex trading?', '["The difference between the bid and ask price", "The total profit made on a trade", "The commission charged by the broker", "The volatility of the market"]'::jsonb, 0, 'The spread is the difference between the buy (ask) and sell (bid) price.'),
    (f_beg, 'Which city is the largest Forex trading center in the world?', '["London", "New York", "Tokyo", "Hong Kong"]'::jsonb, 0, 'London handles roughly 40% of all Forex transactions worldwide.'),
    (f_beg, 'What is "Leverage" in trading?', '["Using borrowed funds to increase position size", "Reducing risk", "Market speed", "A technical indicator"]'::jsonb, 0, 'Leverage allows you to control a large position with a small amount of capital.'),
    (f_beg, 'In the pair GBP/USD, which is the "Base" currency?', '["GBP", "USD", "Both", "Higher value one"]'::jsonb, 0, 'The first currency in a pair is the base currency.'),
    (f_beg, 'A "Bullish" market sentiment means prices are expected to:', '["Rise", "Fall", "Stay same", "Be volatile"]'::jsonb, 0, 'Bulls push prices up.'),
    (f_beg, 'What is a "Lot" in Forex?', '["A standardized unit of transaction size", "A time frame", "Charting software", "A group of traders"]'::jsonb, 0, 'Standard lots are typically 100,000 units of the base currency.'),
    (f_beg, 'Which of these is a technical analysis tool?', '["Moving Average", "GDP Report", "Interest Rate", "Earnings"]'::jsonb, 0, 'Moving averages help identify trends based on past prices.');

    -- STOCKS BEGINNER
    INSERT INTO ai_quiz_questions (quiz_id, question, options, correct_option, explanation) VALUES 
    (s_beg, 'What does "IPO" stand for?', '["Initial Public Offering", "Internal Profit Option", "International Price Order", "Instant Payment"]'::jsonb, 0, 'IPO is when a company first sells shares to the public.'),
    (s_beg, 'Which index represents the 500 largest US companies?', '["S&P 500", "Dow Jones", "Nasdaq 100", "Russell 2000"]'::jsonb, 0, 'The Standard & Poor 500 is a key market benchmark.'),
    (s_beg, 'What is a "Dividend"?', '["A portion of profits paid to shareholders", "The cost of buying stock", "A market crash", "Tax on sales"]'::jsonb, 0, 'Dividends are a way companies share profits with investors.'),
    (s_beg, 'What is "Market Capitalization"?', '["Total value of company shares", "Price of one share", "Total debt", "Number of employees"]'::jsonb, 0, 'Market Cap = Share Price x Total Shares.'),
    (s_beg, 'What is a "Blue Chip" stock?', '["Stock in a large, well-established company", "Penny stock", "Losing stock", "Casino stock"]'::jsonb, 0, 'Blue chips are reliable, large-cap companies like Apple or Microsoft.'),
    (s_beg, 'What is the "P/E Ratio"?', '["Price-to-Earnings Ratio", "Profit-to-Expense", "Price-to-Equity", "Percentage-of-Earnings"]'::jsonb, 0, 'P/E measures a company current share price relative to its per-share earnings.'),
    (s_beg, 'A "Bear Market" is characterized by:', '["Falling prices and pessimism", "Rising prices", "Stable volume", "Winter trading"]'::jsonb, 0, 'Bears pull prices down.'),
    (s_beg, 'Which exchange is known for technology stocks?', '["NASDAQ", "NYSE", "LSE", "JPX"]'::jsonb, 0, 'NASDAQ hosts most major tech giants.'),
    (s_beg, 'What is a "Ticker Symbol"?', '["Unique letters representing a stock", "Sound of a drop", "Technical indicator", "Founder name"]'::jsonb, 0, 'For example, TSLA for Tesla or AAPL for Apple.'),
    (s_beg, 'What is a "Share"?', '["A unit of ownership in a company", "A loan", "The profit", "A contract"]'::jsonb, 0, 'Owning a share means you own a piece of the corporation.');

    -- CRYPTO BEGINNER
    INSERT INTO ai_quiz_questions (quiz_id, question, options, correct_option, explanation) VALUES 
    (c_beg, 'What is the "Blockchain"?', '["A decentralized digital ledger", "A bank vault", "A browser", "Mining hardware"]'::jsonb, 0, 'Blockchain is a distributed database that maintains a growing list of records.'),
    (c_beg, 'Who is the anonymous creator of Bitcoin?', '["Satoshi Nakamoto", "Vitalik Buterin", "Elon Musk", "Charlie Lee"]'::jsonb, 0, 'The identity of Satoshi remains unknown.'),
    (c_beg, 'What is "Mining" in crypto?', '["Verifying transactions for rewards", "Digging in ground", "Buying from exchange", "Storing in safe"]'::jsonb, 0, 'Miners use hardware to solve complex puzzles and secure the network.'),
    (c_beg, 'What is a "Wallet" in cryptocurrency?', '["Tool to manage digital keys", "Leather pouch", "Bank account", "Transaction list"]'::jsonb, 0, 'Wallets store private keys, not the coins themselves.'),
    (c_beg, 'What does "HODL" mean?', '["Holding for long term", "Highly Organized Ledger", "Fast transaction", "Buying much"]'::jsonb, 0, 'HODL originated from a typo for "hold" on a Bitcoin forum.'),
    (c_beg, 'What is the maximum supply of Bitcoin?', '["21 Million", "100 Million", "Infinite", "1 Billion"]'::jsonb, 0, 'Bitcoin has a hard cap of 21,000,000 coins.'),
    (c_beg, 'What is an "Altcoin"?', '["Any crypto other than Bitcoin", "Aluminum coin", "Scam coin", "Payment only coin"]'::jsonb, 0, 'Altcoins include Ethereum, Solana, and thousands of others.'),
    (c_beg, 'What is a "Stablecoin"?', '["Crypto pegged to an asset like USD", "Never loses value", "Gold backed only", "Non-tradable"]'::jsonb, 0, 'USDT and USDC are popular stablecoins pegged to the US Dollar.'),
    (c_beg, 'What is the "Genesis Block"?', '["The first block of a blockchain", "Rich block", "Hacked block", "Last block"]'::jsonb, 0, 'Block 0 of the Bitcoin blockchain was mined on Jan 3, 2009.'),
    (c_beg, 'What is "DeFi"?', '["Decentralized Finance", "Digital File", "Deferred Interest", "Detailed Forex"]'::jsonb, 0, 'DeFi removes intermediaries from financial transactions.');

    -- COMMODITIES BEGINNER
    INSERT INTO ai_quiz_questions (quiz_id, question, options, correct_option, explanation) VALUES 
    (m_beg, 'What is a "Commodity"?', '["Basic good like oil or gold", "Electronic device", "Bank stock", "Service"]'::jsonb, 0, 'Commodities are interchangeable goods used as inputs in production.'),
    (m_beg, 'Which commodity is known as a "Safe Haven"?', '["Gold", "Oil", "Copper", "Coffee"]'::jsonb, 0, 'Investors flock to gold during times of economic uncertainty.'),
    (m_beg, 'What is "Crude Oil"?', '["Unrefined petroleum", "Cooking oil", "Gasoline", "Machine oil"]'::jsonb, 0, 'Crude oil is the raw material for gasoline and plastics.'),
    (m_beg, 'What are "Hard Commodities"?', '["Natural resources mined or extracted", "Difficult to trade", "Agri products", "Fixed price"]'::jsonb, 0, 'Hard commodities include energy and metals.'),
    (m_beg, 'What are "Soft Commodities"?', '["Agri products or livestock", "Gold and silver", "Digital currencies", "Fragile goods"]'::jsonb, 0, 'Soft commodities are grown, not mined.'),
    (m_beg, 'Which city is the hub for Gold trading?', '["London", "Chicago", "Tokyo", "Paris"]'::jsonb, 0, 'The London Bullion Market is the global center for OTC gold.'),
    (m_beg, 'What is a "Futures Contract"?', '["Agreement to buy/sell at set price in future", "Fortune teller contract", "Prediction", "Insurance"]'::jsonb, 0, 'Futures are used by producers to lock in prices.'),
    (m_beg, 'Which of these is a Soft Commodity?', '["Wheat", "Silver", "Crude Oil", "Platinum"]'::jsonb, 0, 'Wheat is an agricultural product.'),
    (m_beg, 'What is "OPEC"?', '["Organization of oil-exporting nations", "Trading platform", "Energy tax", "Clean energy committee"]'::jsonb, 0, 'OPEC manages oil supply to influence global prices.'),
    (m_beg, 'How is Gold typically measured?', '["Troy Ounces", "Kilograms", "Pounds", "Liters"]'::jsonb, 0, 'One troy ounce is approximately 31.1 grams.');
    ...
    (s_int, 'What is "Penny Stock"?', '["High-risk stock at very low price", "Stock worth one penny", "Penny dividends", "UK only stock"]'::jsonb, 0, 'Penny stocks are highly speculative.'),
    ...
    (s_int, 'What is "Limit Order"?', '["Order to buy/sell at specific price or better", "Immediate order", "Max loss amount", "Share restriction"]'::jsonb, 0, 'Limit orders give you price control.');
    ...
    (c_int, 'What is "Whitepaper"?', '["Document explaining project goals and tech", "Blank paper", "Legal contract", "Owner list"]'::jsonb, 0, 'Bitcoin whitepaper was released by Satoshi in 2008.');
    ...
    -- [REMAINING BATCHES INJECTED BELOW]
    ...
    (c_beg, 'What is a "Private Key"?', '["A secret code used to authorize transactions", "Your login name", "Your public address", "A news group"]'::jsonb, 0, 'Whoever has the private key owns the crypto.'),
    ...
    (c_beg, 'What is a "Stablecoin"?', '["A crypto pegged to a stable asset like USD", "A coin that never moves", "A news headline", "A gold-backed coin only"]'::jsonb, 0, 'Stablecoins provide a way to stay in crypto without volatility.'),
    ...
    (m_int, 'What is "Base Metal"?', '["Metals used in industry like copper and zinc", "Precious metals", "Old metals", "No value metals"]'::jsonb, 0, 'Base metals are used in manufacturing and construction.'),
    ...
    (c_eli, 'What is "MEV" (Maximal Extractable Value)?', '["Profit from reordering transactions within a block", "Total value", "A news headline", "A mining reward"]'::jsonb, 0, 'MEV is often extracted by searchers and validators.'),
...
-- 4. SYNC COLUMNS FOR FRONTEND FILTERING
UPDATE ai_quiz_questions aq
SET module = q.module, level = q.level
FROM quizzes q
WHERE aq.quiz_id = q.id;

...
const TradingDisclaimer = ({ isPremiumTheme = false }: TradingDisclaimerProps) => {
  return (
    <div className={`mt-12 py-6 border-t ${
      isPremiumTheme 
        ? 'border-premium-gold/10 bg-premium-panel/30' 
        : 'border-ice bg-white/30'
    }`}>
      <div className="container mx-auto px-6 max-w-[1440px]">
        <div className={`flex items-start gap-3 text-xs leading-relaxed ${
          isPremiumTheme ? 'text-premium-text-muted/70' : 'text-ocean-muted/70'
        }`}>
          <AlertTriangle className={`w-4 h-4 flex-shrink-0 mt-0.5 ${
            isPremiumTheme ? 'text-premium-gold/50' : 'text-aqua/50'
          }`} />
          <p>
